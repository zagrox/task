...
# Purpose: Push all versions to the git repository in order
...
main() {
  ...
  echo "Found the following versions to push:"
  echo "$versions" | sed 's/^/v/'
  echo ""
  
  read -p "Continue with pushing all versions to $REMOTE_NAME/$BRANCH_NAME? (y/n): " confirm
  if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
    echo "Operation cancelled."
    exit 0
  fi
  
  # Push each version tag
  echo "Pushing all version tags to $REMOTE_NAME..."
  success_count=0
  fail_count=0
  
  while IFS= read -r version; do
    if [ -n "$version" ]; then  # Skip empty lines
      echo "Pushing tag v$version to $REMOTE_NAME..."
      if git push "$REMOTE_NAME" "v$version" 2>/dev/null; then
        echo "✅ Successfully pushed tag v$version to $REMOTE_NAME"
        ((success_count++))
      else
  ...
  echo "✅ Completed pushing tags: $success_count successful, $fail_count failed"
}
